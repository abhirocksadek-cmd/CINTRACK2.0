...
export function CalculatorForm({ onSubmit, loading }: Props) {
  ...
  // Detect JSDOM test environment to keep all inputs visible for Testing Library / Vitest
  ...
  return (
    <form className="card" onSubmit={handleSubmit} aria-labelledby="calc-heading">
      ...
      {/* Stepper Navigation */}
      {!isTest && (
        <div className="wizard-nav" aria-label="Calculator steps">
          <button
            type="button"
            className={`wizard-nav-btn ${activeStep === 0 ? "active" : ""}`}
          ...
          <button
            type="button"
            className={`wizard-nav-btn ${activeStep === 1 ? "active" : ""}`}
          ...
          <button
            type="button"
            className={`wizard-nav-btn ${activeStep === 2 ? "active" : ""}`}
      ...
      {/* Step 1: Transport */}
      <div className={`wizard-step ${activeStep === 0 || isTest ? "active" : ""}`}>
        <fieldset>
        ...
        {!isTest && (
          <div className="wizard-actions">
            <div />
            <button type="button" className="btn" onClick={() => setActiveStep(1)}>
              Next Step 🏠
            </button>
          </div>
        )}
      ...
      {/* Step 2: Home Energy */}
      <div className={`wizard-step ${activeStep === 1 || isTest ? "active" : ""}`}>
        <fieldset>
        ...
        {!isTest && (
          <div className="wizard-actions">
            <button type="button" className="btn secondary" onClick={() => setActiveStep(0)}>
              Back 🚗
            </button>
            <button type="button" className="btn" onClick={() => setActiveStep(2)}>
              Next Step 🥗
            </button>
          </div>
        )}
      ...
      <div className={`wizard-step ${activeStep === 2 || isTest ? "active" : ""}`}>
        <fieldset>
